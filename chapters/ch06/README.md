...
*Enums* give a way of saying a value is one of a possible set of values. For example, a `Rectangle` is one of a set of possible shapes that also includes `Circle` and `Triangle`. See [`ch05/rectangles`](../ch05/rectangles) for a toy example with shapes.
